...
import type { GalleryPhoto } from "@/modules/landing-page/types";
          ...
          <div className="overflow-hidden" ref={emblaRef}>
            <ul className="flex -ml-4">
              {galleryImages.map((image) => (
                <li key={image.id} className="shrink-0 pl-4">
                  <button
                    type="button"
                    onClick={() => setActiveImage(image)}
                    className="group relative block h-72 w-fit overflow-hidden rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-400 focus-visible:ring-offset-2 sm:h-80 lg:h-96"
                  >
                    <Image
                      src={image.src}
                      alt={image.alt}
      ...
      >
        {activeImage ? (
          <Image
            ...
            sizes="92vw"
            className="mx-auto h-auto max-h-[85vh] w-auto max-w-full rounded-2xl"
          />
        ) : null}
      </Modal>
    </section>
